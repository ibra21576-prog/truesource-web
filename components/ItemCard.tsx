...
function timeAgo(ts: string) {
  ...
  if (d < 86400000) return `${Math.round(d/3600000)} Std.`
  return new Date(ts).toLocaleDateString('de-DE', { day:'numeric', month:'short' })
}

const P: Record<string, { label: string; color: string; bar: string }> = {
  vinted:        { label: 'Vinted',        color: '#3df5c8', bar: '#3df5c8' },
  ebay:          { label: 'eBay',          color: '#fbbf24', bar: '#fbbf24' },
  kleinanzeigen: { label: 'Kleinanzeigen', color: '#fb923c', bar: '#fb923c' },
}
    ...
    >
      ...
      <div style={{
        ...
        {item.image
          ...
          : <svg width="24" height="24" fill="none" stroke="var(--border2)" strokeWidth="1.5" viewBox="0 0 24 24">
      ...
      <div style={{ flex: 1, minWidth: 0, padding: '14px 0', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 5 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          {isNew && (
            <span className="badge badge-new pulse" style={{ fontSize: 10, letterSpacing: '0.06em' }}>✦ NEU</span>
          )}
        </div>
