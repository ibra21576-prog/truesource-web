    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div className="page">
        <div style={{ marginBottom: 28 }}>
          <h1 style={{ fontSize: 24, fontWeight: 800, color: 'var(--text)', margin: 0, letterSpacing: '-0.02em' }}>Searches</h1>
          <p style={{ fontSize: 13, color: 'var(--text3)', marginTop: 4 }}>{searches.length} search{searches.length !== 1 ? 'es' : ''} configured</p>
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {loading ? (
              ...
              <div style={{ textAlign: 'center', padding: '48px 24px', background: 'var(--card)', border: '1.5px solid var(--border)', borderRadius: 14 }}>
                <p style={{ fontWeight: 600, fontSize: 15, color: 'var(--text)', marginBottom: 6 }}>No searches yet</p>
                ...
                <div key={s.id} className="anim-in" style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0, padding: '12px 14px' }}>
                    ...
                    {(s.min_price || s.max_price) && (
                      <p style={{ fontSize: 11, color: 'var(--text3)', marginTop: 3 }}>{s.min_price ?? 0}€ – {s.max_price ?? '∞'}€</p>
                    )}
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 2, padding: '0 8px', flexShrink: 0 }}>
                    <button onClick={() => toggle(s)} title={s.enabled ? 'Pause' : 'Resume'} className="btn-ghost"
                    ...
                    <button onClick={() => remove(s.id)} title="Delete" className="btn-ghost" style={{ padding: '8px' }}
