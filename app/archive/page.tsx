    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div className="page">
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, marginBottom: 24, flexWrap: 'wrap' }}>
          <div>
            <h1 style={{ fontSize: 24, fontWeight: 800, color: 'var(--text)', margin: 0, letterSpacing: '-0.02em' }}>Archiv</h1>
            <p style={{ fontSize: 13, color: 'var(--text3)', marginTop: 4 }}>{filtered.length} von {items.length} Listings</p>
          </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ position: 'relative', flex: 1, minWidth: 180, maxWidth: 300 }}>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Suche im Archiv…" style={{ paddingLeft: 36 }} />
        ...
        {loading ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '80px 0', gap: 14 }}>
            ...
            <p style={{ color: 'var(--text3)', fontSize: 14 }}>Lade Archiv…</p>
          </div>
        ) : filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '72px 24px', background: 'var(--card)', border: '1.5px solid var(--border)', borderRadius: 14 }}>
            ...
            <p style={{ fontWeight: 700, fontSize: 15, color: 'var(--text)', marginBottom: 6 }}>Archiv leer</p>
            <p style={{ fontSize: 13, color: 'var(--text3)' }}>{search ? 'Keine Treffer für diese Suche.' : 'Listings werden hier automatisch gespeichert.'}</p>
          </div>
        ) : (
